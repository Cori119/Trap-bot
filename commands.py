...
class commands:
	...
	async def dailies(self, ctx):
		...
		embed.add_field(name = "ðŸ“† " + str(datetime.now().month) + "/" + str(datetime.now().day) + " " + calendar.day_name[date.today().weekday()] + " Dailies", value = dailies[int(datetime.today().strftime('%w'))], inline=True)
		try:
			await self.bot.send_message(ctx.message.channel, embed=embed)
		except Exception as e:
			await self.bot.say("I need embed permissions to reply properly.")
		...
		#await self.bot.send_message(ctx.message.channel, "ðŸ“† **" + str(datetime.now().month) + "/" + str(datetime.now().day) + " " + calendar.day_name[date.today().weekday()]+ "**\n")
		#await self.bot.say("ðŸ“† **" + str(datetime.now().month) + "/" + str(datetime.now().day) + " " + calendar.day_name[date.today().weekday()]+ "**\n" + dailies[int(datetime.today().strftime('%w'))])
	...
	async def f2(self, ctx, *, username : str = None):
		...
		if username:
			...
			if ctx.message.server == server:
				await self.bot.say("ðŸ”— http://na-bns.ncsoft.com/ingame/bs/character/profile?c=" + username)
				await self.bot.say("ðŸ”— http://na-bns.ncsoft.com/ingame/bs/character/profile?c=ginxo")
	...
	async def ynpoll(self, ctx, *, question : str = None):
		...
		if question:
			...
			if wait.content == "0":
				...
				await self.bot.add_reaction(poll_message, 'ðŸ‘')
				await self.bot.add_reaction(poll_message, 'ðŸ‘Ž')
	...
	async def roll(self, ctx, *, bounds : str = None):
		...
		if bounds and "-" in ctx.message.content:
			...
			if result == "1":
				await self.bot.say("ðŸŽ² You rolled **" + result + "**. Nice roll, nerd.")
			elif result == "69":
				await self.bot.say("ðŸŽ² You rolled **" + result + "**. <:feelsReceive:388769216343310339>")
			else:
				await self.bot.say("ðŸŽ² You rolled **" + result + "**.")
		elif bounds:
	...
	async def magic8ball(self, ctx):
		...
		if len(ctx.message.content) > len("!8ball "):
			await self.bot.say("ðŸŽ± " + results[answer])
		else:
